...
def test_first_child_token_match():
    ...
    assert first_child_token_match(tokens[0]) == TokenInfo(
        type=token.COMMENT, string="# comment 1", start=(2, 0), end=(2, 11), line="# comment 1\n"
    )
    assert first_child_token_match(tokens[1]) == TokenInfo(
        type=token.NAME, string="def", start=(4, 0), end=(4, 3), line="def foo():  # comment 2\n"
    )
